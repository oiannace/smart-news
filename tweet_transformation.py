import json
import lib.tweet_processing_functions
import boto3
...
def lambda_handler(event, context):
    ...
    users_following_ids = json.loads(bucket.download_file('user_ids.txt'))
    
    users_following_tweets = {}
    for user_id in users_following_ids:
        users_following_tweets[user_id] = json.loads(bucket.download_file(f'{user_id}_user_tweets.txt'))
